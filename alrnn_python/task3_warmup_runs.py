#!/usr/bin/env python3
"""
Task 3.5v2: Two-Phase Training — L1 Warm-up
============================================
Phase 1 (epoch 0-500): Pure MSE — learn attractor geometry
Phase 2 (epoch 500-1000): MSE + L1 — sparsify while preserving attractor

Conditions:
  F: M=20, L1(activated), warmup@500  (our best attempt)
  G: M=20, NO L1 baseline             (pure reconstruction)

5 seeds × 2 conditions = 10 runs
Estimated: ~20 min/run × 10 = ~3.3 hours
...
def train_sh(model, dataset, optimizer, scheduler, loss_fn,
             ...
             grad_clip=10.0, l1_warmup_epochs=0, verbose=True):
    """Training loop with gradient clipping and L1 warm-up."""
    model.train()
    ...
    for e in iterator:
        ...
        # L1 warm-up: no sparsity during first phase
        ...
        for _ in range(batches_per_epoch):
            ...
            loss = loss_fn(z_hat[:, :, :model.N], y) + effective_lambda * l1_pen
            loss.backward()
        ...
        if verbose and hasattr(iterator, 'set_postfix'):
            iterator.set_postfix(loss=avg_loss, phase='L1' if e >= l1_warmup_epochs else 'MSE')
...
CONDITIONS = {
    'F': {'label': 'M=20, L1(act) warmup@500', 'M': 20, 'P': 2, 'N': 3,
          'dropout_p': 0.0, 'lambda_l1': 1e-4, 'l1_mode': 'activated',
          'l1_warmup_epochs': 500},
    'G': {'label': 'M=20, NO L1 (baseline)', 'M': 20, 'P': 2, 'N': 3,
          'dropout_p': 0.0, 'lambda_l1': 0.0, 'l1_mode': 'none',
          'l1_warmup_epochs': 0},
}
...
def main():
    output_dir = os.path.join(script_dir, 'experiments', 'task3_warmup')
    ...
    print("=" * 70)
    print(f"  Phase 1 (0-500):   Pure MSE — learn attractor geometry")
    print(f"  Phase 2 (500-1000): MSE + L1 — sparsify")
    print(f"  Conditions  : F (L1 warmup@500), G (no L1 baseline)")
    print(f"  Seeds       : {SEEDS}")
    ...
    for cond_key, cond in CONDITIONS.items():
        for seed in SEEDS:
            ...
            if cond['l1_warmup_epochs'] > 0:
                print(f"  L1 warmup: OFF for epochs 0-{cond['l1_warmup_epochs']-1}, "
                      f"ON from epoch {cond['l1_warmup_epochs']}")
            print(f"{'─' * 70}")
            ...
            hist = train_sh(
                ...
                l1_warmup_epochs=cond['l1_warmup_epochs'],
                verbose=True,
            )
    ...
    # Summary
    print(f"\n{'=' * 70}")
    print("  TASK 3.5v2 FINAL SUMMARY")
    print(f"{'=' * 70}")

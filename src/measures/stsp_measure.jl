...
"""
    state_space_distance(ground_truth, generated)

Compute the D_stsp measure given two time series `X` and `X̃` using a binning approach. 
...
by `[-2σ, 2σ]ᴺ` (equal to the Python implementation).
...
@inbounds function state_space_distance(
    X::AbstractMatrix,
    X̃::AbstractMatrix,
    σ²::Real;
    max_T::Int = 10000,

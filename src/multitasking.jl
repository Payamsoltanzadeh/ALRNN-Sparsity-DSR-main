...
function main_routine(args::AbstractDict)
    ...
    D = SymbolicDSR.Training.Dataset(args["path_to_data"]; device = device)

    # initialize rnn model with wanted structure
    alrnn = initialize_model(args, D) |> device  # model
    O = initialize_observation_model(args, D) |> device  # Observation model
    ...
    save_model([alrnn, O], joinpath(save_path, "checkpoints", "model_0.bson"),)

    # store hypers
    store_hypers(args, save_path)

    train_!(alrnn, O, D, opt, args, save_path)
